...
export function getGuestCards(): GuestCard[] {
  //gets the local storage flashcards
...
export function saveGuestCards(cards: GuestCard[]) {
  //function that saves a flashcard to an individual guestkey
  localStorage.setItem(guestKey, JSON.stringify(cards));
}

export function createGuestCard(
  front: string,
  back: string,
  section?: string,
): //function that creates a flashcard
{ ok: true; cards: GuestCard[] } | { ok: false; reason: "limit_reached" } {
...
export function deleteGuestCard(id: string): GuestCard[] {
  //removes a flashcard and saves new updated list into local storage
  const cards = getGuestCards();
...
export function editGuestCard(
  id: string,
  front: string,
  back: string,
  section?: string,
): //edit flashcard
{ ok: true; cards: GuestCard[] } | { ok: false; reason: "card_not_found" } {
  //no id gives error
  const cards = getGuestCards();
  ...
  const editedGuestCard = cards.map((c) =>
    c.id === id
      ? {
