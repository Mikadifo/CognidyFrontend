...
async function createCard(e: React.FormEvent) {
    ...
    try {
      setSaving(true);
      setErr(null);
      const res = await fetch(`${API}/flashcards`, {  // post/create route from backend
        method: "POST",
      ...
      setCards((prev) => (prev ? [{ id: data.id, front: data.front, back: data.back }, ...prev] : [data]));
      setFront("");
      setBack("");
    } catch (e: any) {
...
async function deleteCard(id: string) {
  ...
  } catch (e: any) { //error handling if card is being set to delete and isn't able to
    setCards(prev);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <form onSubmit={createCard} className="rounded-2xl border border-black/10 bg-white p-4 shadow-sm"> 
