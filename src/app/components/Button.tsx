...
export const Button: FC<ButtonProps> = ({
  ...
  const styles = `${className} font-poppins font-normal text-base px-4 py-2 md:px-6 md:py-3 rounded-sm ${variant === "filled" ? `bg-dark text-white ${disabled ? "" : "hover:opacity-80"}` : `border border-dark text-dark ${disabled ? "bg-dark-08" : "hover:bg-dark-08"}`} ${Icon && "flex gap-2 items-center justify-center"} ${disabled ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`;
  const children = (
    <>
      {props.children}
