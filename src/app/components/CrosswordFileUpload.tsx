...
export function CrosswordFileUpload({ onPuzzleGenerated }: CrosswordFileUploadProps) {
...
const handleUpload = async () => {
  ...
  try {
    ...
    console.log('Calling:', `${process.env.NEXT_PUBLIC_API_URL}/api/crosswords/generate`);

    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/crosswords/generate`, {
      method: 'POST',
      body: formData,
    });
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      if (result.success) {
        ...
        sessionStorage.setItem(`crossword_${result.puzzle.metadata.puzzleID}`, JSON.stringify(result.puzzle));
      ...
      setStatus(`Server error: Received ${contentType} instead of JSON. Check console for details.`);
    }
  } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-4 text-center">Create Crossword Puzzle</h2>
      <p className="text-gray-600 mb-6 text-center">
        Upload your study notes and we'll generate a crossword puzzle to help you learn!
      </p>
